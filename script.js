...
const criarDiasSelecao = () => {
  const diasSelecao = [];
  const anoAtual = 2024; // Por exemplo, você pode ajustar isso conforme necessário
  const meses = [
  ...
  for (let mes = 0; mes < 12; mes++) {
    ...
    for (let dia = 1; dia <= ultimoDia; dia++) {
      ...
      const diaFormatado = `${String(dia).padStart(2, '0')} de ${meses[mes]}`;
      diasSelecao.push(`<option value="${data.toISOString().split('T')[0]}">${diaFormatado}</option>`);
    }
  }
